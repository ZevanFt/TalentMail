...
const { t } = useI18n()
useHead({ title: computed(() => file.value ? `${file.value.original_filename} - ${t('share.shareLabel')} - ${config.appName}` : `${t('share.fileShare')} - ${config.appName}`) })
...
const loadShare = async () => {
  ...
  try {
    ...
    if (e.data?.detail === '需要密码') {
      ...
      error.value = t('share.wrongPassword')
    } else {
      error.value = e.data?.detail || t('share.invalidLink')
    }
  } finally {
    loading.value = false
  }
  ...
  <div class="min-h-screen bg-gray-100 dark:bg-bg-dark flex items-center justify-center p-4">
    <div class="bg-white dark:bg-bg-panelDark rounded-xl shadow-lg p-6 w-full max-w-md">
      ...
      <div v-else-if="needPassword" class="text-center">
        ...
        <h2 class="text-lg font-bold mb-2">{{ t('share.needPassword') }}</h2>
        <input v-model="password" type="password" :placeholder="t('share.passwordPlaceholder')" class="w-full px-4 py-2 border rounded-lg mb-3 dark:bg-gray-700 dark:border-gray-600" @keyup.enter="loadShare" />
        <div v-if="error" class="text-red-500 text-sm mb-3">{{ error }}</div>
        <button @click="loadShare" class="w-full py-2 bg-primary text-white rounded-lg hover:bg-primary-hover">{{ t('share.verify') }}</button>
      </div>
      ...
      <div v-else-if="file" class="text-center">
        ...
        <button @click="download" :disabled="downloading" class="w-full py-3 bg-primary text-white rounded-lg hover:bg-primary-hover flex items-center justify-center gap-2 disabled:opacity-50">
          ...
          {{ downloading ? t('share.downloading') : t('share.downloadFile') }}
        </button>
        <p class="text-xs text-gray-400 mt-4">{{ t('share.downloadedCount', { n: file.download_count }) }}</p>
      </div>
    </div>
