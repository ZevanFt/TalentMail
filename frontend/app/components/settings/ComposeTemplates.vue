...
const validateForm = (): boolean => {
  formErrors.name = ''
  if (!form.name.trim()) {
    formErrors.name = t('settings.composeTemplates.nameRequired')
    return false
  }
  return true
}
...
const loadTemplates = async () => {
  ...
  try {
    ...
    loadError.value = e.data?.detail || t('settings.composeTemplates.loadFailed')
...
const save = async () => {
  ...
  try {
    ...
    toast.success(editingTemplate.value ? t('settings.composeTemplates.updated') : t('settings.composeTemplates.created'))
    await loadTemplates()
  } catch (e: any) {
    console.error('保存模板失败', e)
    toast.error(e.data?.detail || t('settings.composeTemplates.saveFailed'))
  } finally {
    saving.value = false
  }
}
...
const remove = async (id: number) => {
  const ok = await confirmDialog({ message: t('settings.composeTemplates.confirmDelete'), type: 'danger' })
  if (!ok) return
  deletingId.value = id
  try {
    await deleteComposeTemplate(id)
    templates.value = templates.value.filter(t => t.id !== id)
    toast.success(t('settings.composeTemplates.deleted'))
  } catch (e: any) {
    console.error('删除模板失败', e)
    toast.error(e.data?.detail || t('settings.composeTemplates.deleteFailed'))
  } finally {
...
<template>
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h2 class="text-lg font-bold text-gray-900 dark:text-white">{{ t('settings.tabs.composeTemplates') }}</h2>
        <p class="text-sm text-gray-500 mt-1">{{ t('settings.composeTemplates.subtitle') }}</p>
      </div>
      <button @click="openModal()"
        ...
        <Plus class="w-4 h-4" /> {{ t('settings.composeTemplates.create') }}
      </button>
    </div>
    ...
    <div v-else-if="loadError" class="text-center py-12">
      ...
      <button @click="loadTemplates" class="px-4 py-2 bg-primary text-white rounded-lg text-sm hover:bg-primary-hover transition-colors">{{ t('common.retry') }}</button>
    </div>

    <!-- Empty -->
    <div v-else-if="templates.length === 0" class="text-center py-12 text-gray-500">
      <FileText class="w-12 h-12 mx-auto mb-3 opacity-30" />
      <p class="text-lg font-medium mb-1">{{ t('settings.composeTemplates.emptyTitle') }}</p>
      <p class="text-sm text-gray-400">{{ t('settings.composeTemplates.emptyDesc') }}</p>
    </div>
        ...
        <div class="flex items-start justify-between gap-3">
          <div class="flex items-start gap-3 min-w-0 flex-1">
            ...
            <div class="min-w-0 flex-1">
              ...
              <div v-if="tmpl.subject" class="text-sm text-gray-600 dark:text-gray-400 mt-0.5 truncate">
                <span class="text-gray-400">{{ t('settings.composeTemplates.subjectLabel') }}</span>{{ tmpl.subject }}
              </div>
          ...
          <div class="flex gap-1 shrink-0">
            <button @click="openModal(tmpl)"
              class="p-1.5 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors" :title="t('settings.automation.edit')">
    ...
    <!-- Create/Edit Modal -->
    <CommonModal v-model="showModal" :title="editingTemplate ? t('settings.composeTemplates.editTitle') : t('settings.composeTemplates.create')" size="lg">
      <div class="space-y-4">
        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            {{ t('settings.composeTemplates.name') }} <span class="text-red-500">*</span>
          </label>
          <input v-model="form.name" :placeholder="t('settings.composeTemplates.namePlaceholder')"
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ t('settings.composeTemplates.subject') }}</label>
          <input v-model="form.subject" :placeholder="t('settings.composeTemplates.subjectPlaceholder')"
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ t('settings.composeTemplates.body') }}</label>
          <textarea v-model="form.body_html" :placeholder="t('settings.composeTemplates.bodyPlaceholder')" rows="8"
          ...
          <p class="text-xs text-gray-400 mt-1">{{ t('settings.composeTemplates.bodyHint') }}</p>
        </div>
      </div>
      <template #footer>
        <button @click="showModal = false"
          class="px-4 py-2 text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg transition-colors text-sm">{{ t('common.cancel') }}</button>
        <button @click="save" :disabled="saving"
          ...
          {{ saving ? t('settings.common.saving') : t('common.save') }}
        </button>
      </template>
    </CommonModal>
  </div>
