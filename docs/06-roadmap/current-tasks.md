...
> ⚠️ 维护状态（2026-03-08）：本文件主要用于历史归档，当前功能完成度与待办优先级请以 `todo-list.md` 为准。
...
- **API 开放平台（MVP）**：已完成 API Key 创建/列表/吊销、Bearer 鉴权与 Scope 校验。
- **自动化接口**：已上线 `/api/automation/temp-mailboxes`、邮件读取与最新验证码提取接口。
- **稳定性能力**：已接入基础限流、API Key 审计日志、创建邮箱幂等（`Idempotency-Key`）。
- **待收尾项**：开发者文档细化、调用示例补全、限流策略分层优化。
...
- ✅ 实现标记垃圾/非垃圾邮件功能
- ✅ 白名单管理（增删查）
- ⚠️ 后台任务训练链路目前为占位实现（真实 `sa-learn` 调用待完成）
