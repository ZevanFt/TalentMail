...
### 1. 邮件与平台能力
- [ ] **垃圾邮件训练闭环**
  - 调用真实 `sa-learn --spam/--ham`
  - 训练结果持久化与失败重试
  - 后台任务可观测日志

- [ ] **API 开放平台（阶段化推进）**
  - 已完成：API Key 管理（创建/列表/吊销）、Scope 鉴权、自动化临时邮箱与验证码接口
  - 已完成：基础限流、审计日志、创建邮箱幂等支持
  - 待完成：开发者文档细化、SDK/脚本示例补全、限流策略分层（按场景）
