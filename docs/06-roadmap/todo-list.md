...
| API 开放平台 | 90% | 基本完成 | Key 管理、鉴权、限流分层、SDK/文档示例已落地；OAuth/开发者门户仍属后续 |
...
| 备份恢复 | 80% | 基本完成 | 脚本 + systemd timer + 管理端 API；备份加密与上传目录备份可再增强 |
| 操作审计 | 70% | 基本完成 | `operation_audit_logs` + 登录/备份流水 + 管理端查询；更多动作点逐步接入 |
| 性能监控 | 60% | 部分完成 | health 含 DB/任务/磁盘/uptime；队列监控与告警待补 |
| 国际化 | 30% | 框架就绪 | 轻量 i18n + 设置切换 + 日历页试点 |
| 日历增强 | 60% | 部分完成 | 周视图 + daily/weekly/monthly 循环；CalDAV/.ics 导出待补 |
...
- [x] **操作日志审计（骨架）** - 2026-03-08
  - `operation_audit_logs` 表 + 写入辅助
  - 登录 / 备份动作已接入
  - 管理端查询 API
  - 待完成：发信/删除/管理动作批量接入、异常行为检测

- [ ] **性能监控系统**
  - [x] health 含 DB / 后台任务 / 磁盘 / uptime
  - [ ] 邮件队列监控
  - [ ] 告警通知（Webhook）
...
- [x] **国际化框架** - 2026-03-08
  - 轻量 i18n composable + zh-CN/en-US 词典
  - 设置页语言切换（localStorage 持久化）
  - 日历页试点；其余页面逐步迁入
