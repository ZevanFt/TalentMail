...
| 操作审计 | 85% | 基本完成 | 登录/备份/发信/删用户/云盘上传/联系人/工作流创建 + 管理端查询 |
| 性能监控 | 75% | 基本完成 | health 含 DB/任务/磁盘/uptime/邮件队列；Webhook 告警已接入 |
| 国际化 | 92% | 进行中 | 在 85% 基础上完成：邮件模块（列表/写信/详情/模板选择/联系人联想）+ 工作流模块（编辑器/教程/我的工作流/全部弹窗）迁入 i18n；剩余：布局侧栏/Header、账号池、云盘/通讯录页内余量、认证页、日历弹窗、编辑器工具栏、common 组件、error 页（约 550 行） |
...
- [ ] **国际化完善**
  ...
  - [ ] 剩余：布局侧栏/Header、账号池页、云盘/通讯录页内剩余文案、认证页、日历弹窗、编辑器工具栏、common 组件、error 页、错误消息本地化
